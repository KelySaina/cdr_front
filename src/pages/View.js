...
const View = () => {
    ...
    const getData = async () => {
        const response = await axios.get(`http://192.168.43.14:5000/aff`)
    ...
    const getNb = async () => {
        const response = await axios.get(`http://192.168.43.14:5000/nb`)
        const data = response.data[0]
        setNbData(data)
    }

    const filter = async (key) => {
        const filteredRooms = tableData.filter((room) =>
            room.src.includes(key) ||
            room.dst.includes(key) ||
            room.duration.toString().includes(key) ||
